...
from data_generator import DataGeneratorPommerman, DataGeneratorPPO
from dqn import DQN
from ppo2 import PPO
from models import Pommer_Q, ActorCritic
...
def train_ppo(ppo1=None, ppo2=None, num_iterations=p.num_iterations, episodes_per_iter=p.episodes_per_iter, augmentors=[], enemy='static:0', max_steps=p.max_steps) :
    ...
    # initialize DQN and data generator
    if ppo1 == None:
        actor_critic1 = ActorCritic(p.p_observable or p.centralize_planes, transform_func)
        ppo1 = PPO(actor_critic1, p.exploration_noise)

    if ppo2 == None:
        actor_critic2 = ActorCritic(p.p_observable or p.centralize_planes, transform_func)
        ppo2 = PPO(actor_critic2, p.exploration_noise)

    data_generator = DataGeneratorPPO(
        p.env,
        augmentors=augmentors
    )
    ...
    for i in range(num_iterations):
        logging.info(f"Iteration {i+1}/{num_iterations} started")
        iteration_stopwatch = Stopwatch(start=True)
        policy1 = ppo1.get_policy()
        policy2 = ppo2.get_policy()

        # generate data an store normalized act counts and win ration
        res, ties, avg_rwd, act_counts, avg_steps = data_generator.generate(episodes_per_iter, policy1, policy2, enemy, ppo1.policy.get_transformer(), 'train', 'train', max_steps)
        act_counts[0] = [act/sum(act_counts[0]) for act in act_counts[0]]
        act_counts[1] = [act/sum(act_counts[1]) for act in act_counts[1]]
        ...
        loss1 = ppo1.update(data_generator.buffers[0])
        loss2 = ppo2.update(data_generator.buffers[1])

        avg_loss=(loss1 + loss2)/2

        data_generator.buffers[0] = {"actions" : [], "states" : [], "rewards" : [], "logprobs" : [], "dones" : []}
        data_generator.buffers[1] = {"actions" : [], "states" : [], "rewards" : [], "logprobs" : [], "dones" : []}
        ...
        if i % p.intermediate_test == p.intermediate_test-1:
            ...
            policy2 = ppo2.get_policy()
            model_save_path = log_dir + "/" + str(i)
            torch.save(ppo1.policy.state_dict(), model_save_path + '_1')
            torch.save(ppo2.policy.state_dict(), model_save_path + '_2')
            logging.info("Saved model to: " + model_save_path)
            data_generator.generate(p.episodes_per_eval, policy1, policy2, enemy, ppo1.policy.get_transformer(), 'train', 'train', max_steps, render=p.render_tests)
            ppo1.set_train(True)
            ppo2.set_train(True)
            logging.debug(f"Test finished after {test_stopwatch.stop()}s")
            logging.info("------------------------")
...
def main(args):
    ...
    parser.add_argument('-a', '--algorithm', type=str, dest="alg", default="dqn", help=f"Algorithm for agent training")
    ...
    if(args.alg == "dqn"):
        ...
        train_ppo(num_iterations=2000, enemy='smart_random', augmentors=[])
