...
class DataGeneratorPommerman:
    ...
    def generate(self, episodes: int, policy1: Callable, policy2: Callable, enemy: str, transformer: Callable,
        ...
        for i_episode in range(episodes):
            ...
            while not done:
                ...
                if p.reward_func == "SkynetReward":
                    skynet_rwds = skynet_reward(obs, act, nobs, fifo, agent_inds, skynet_reward_log, done, bomb_tracker)
                for i in range(self.player_agents_n):
                    agt: TrainAgent = agent_list[agent_inds[i]]
                    if p.reward_func == "SkynetReward":
                        agt_rwd = skynet_rwds[agent_inds[i]]
                        '''
                        act_no_msg = act[agent_inds[i]][0] if type(act[agent_inds[i]]) == list else act[agent_inds[i]]
                        if act_no_msg == 5:
                            bomb_rwd = woods_close_to_bomb_reward(obs[agent_inds[i]], agt.position, obs[agent_inds[i]]['blast_strength'], agent_ids)
                            agt_rwd += bomb_rwd
                            skynet_reward_log[i][1] += bomb_rwd
                        '''
            ...
            avg_steps += steps_n
            for i in range(self.player_agents_n):
                agent_list[agent_inds[i]].reset_agent()
        ...
        if p.reward_func == "SkynetReward":
            logging.info(
                f"Skynet Reward split agent 1:  Win/Loss: {skynet_reward_log[0][3]}, Kills: {skynet_reward_log[0][0]}, Items: {skynet_reward_log[0][1]},"
                        f" Steps(FIFO): {skynet_reward_log[0][2]}, Deaths: {skynet_reward_log[0][4]}")
            logging.info(
                f"Skynet Reward split agent 2: Win/Loss: {skynet_reward_log[1][3]}, Kills: {skynet_reward_log[1][0]}, Items: {skynet_reward_log[1][1]},"
                        f" Steps(FIFO): {skynet_reward_log[1][2]},  Deaths: {skynet_reward_log[1][4]}")
