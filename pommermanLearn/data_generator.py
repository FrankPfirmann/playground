...
class DataGeneratorPommerman:
    def __init__(self, env, augmentors: list=[], algo="dqn")-> None:
        """
    ...
    def _init_agent_list(self, agent1, agent2, policy1, policy2, enemy, setposition=False):
        ...
        for i in range(0, self.agents_n):
            ...
            if agent_str.startswith('static'):
                ...
                agent_list[i] = TrainAgent(policy1, self.algo)
            elif agent_str == 'train' and i == agent_ind + 2:
                agent_list[i] = TrainAgent(policy2, self.algo)
...
class DataGeneratorPPO:

    def __init__(self, env, augmentors: list=[], algo="ppo")-> None:
        """
        Create a new DataGenerator instance.
        ...
        self.agents_n = 2 if self.env == 'OneVsOne-v0' or self.env.startswith("custom") else 4
        self.player_agents_n = int(self.agents_n/2)
        self.buffers = [{"actions" : [], "states" : [], "rewards" : [], "logprobs" : [], "dones" : []} for _ in range(self.player_agents_n)]
        self.idxs = [0, 0]
        self.augmentors = augmentors
    ...
    def generate(self, episodes: int, policy1: Callable, policy2: Callable, enemy: str, transformer: Callable, agent1, agent2, max_steps: int=p.max_steps, render: bool=False) -> tuple:
        ...
        for i_episode in range(episodes):
            ...
            while not done:
                ...
                if type(act[agent_inds[0]]) == tuple:

                    logprobs[0] = act[agent_inds[0]][1]
                    states[0] = act[agent_inds[0]][2]
                    act[agent_inds[0]] = act[agent_inds[0]][0].item()
                
                if type(act[agent_inds[1]]) == tuple:

                    logprobs[1] = act[agent_inds[1]][1] 
                    states[1] = act[agent_inds[1]][2] 
                    act[agent_inds[1]] = act[agent_inds[1]][0].item()
                ...
                for i in range(self.player_agents_n):
                    ...
                    if alive[i]:
                        ...
                        self.buffers[i]['actions'].append(torch.Tensor([act[agent_inds[i]]]))
                        self.buffers[i]['states'].append(states[i])
                        self.buffers[i]['logprobs'].append(logprobs[i])
                        self.buffers[i]['rewards'].append(agt_rwd*100)
