import torch
import torch.nn as nn
import torch.nn.functional as F


class DQN_Q(nn.Module):
    def __init__(self, obs_size, act_size, hidden_size):
        super(DQN_Q, self).__init__()

        self.linear1 = nn.Linear(obs_size, hidden_size)
        self.linear2 = nn.Linear(hidden_size, hidden_size)
        self.linear3 = nn.Linear(hidden_size, act_size)

    def forward(self, obs):
        x1 = F.relu(self.linear1(obs))
        x1 = F.relu(self.linear2(x1))
        x1 = self.linear3(x1)
        return x1


class Pommer_Q(nn.Module):
    def __init__(self):
        super(Pommer_Q, self).__init__()

        self.conv = nn.Sequential(
            nn.Conv2d(in_channels=12, out_channels=32, kernel_size=(2, 2), stride=(1, 1)),
            nn.ReLU(),
            nn.Conv2d(in_channels=32, out_channels=64, kernel_size=(4, 4), stride=(1, 1)),
            nn.ReLU(),
            nn.Conv2d(in_channels=64, out_channels=128, kernel_size=(4, 4), stride=(1, 1)),
            nn.ReLU(),
            nn.Conv2d(in_channels=128, out_channels=256, kernel_size=(4, 4), stride=(1, 1)),
            nn.ReLU()
        )

        self.linear = nn.Sequential(
            nn.Linear(in_features=256 , out_features=128),
            nn.ReLU(),
            nn.Linear(in_features=128, out_features=64),
            nn.ReLU(),
            nn.Linear(in_features=64, out_features=6)
        )


    def forward(self, obs):
        x = self.conv(obs).squeeze()
        x = self.linear(x).unsqueeze(0)
        return x
